## HALLO

I'm Cole (or Coal). I am in college for Computer Science.

* Programming since 2020
* 🖥️ NixOS & [niri](https://github.com/niri-wm/niri)
* 🔧 Rust is my favorite language.
* 🌐 Svelte(Kit) advocate.
* 📫 Contact me by email via `theburnrock [at] proton [dot] me` or on Discord at
  `thepoultryman`.
* I love making systems of any flavor in **rust** and **zig**
* I make Minecraft mods
* ECS is literally one my favorite things ever

### Projects

<!--Who knew markdown tables were made by the devil.-->

|Minecraft Mods|Rust|
|--------------|----|
| • [Crops Love Rain](https://codeberg.org/ThePoultryMan/Crops-Love-Rain)<br>• [Lanterns Belong on Walls](https://codeberg.org/ThePoultryMan/Lanterns-Belong-On-Walls)<br>• [Particle Moths](https://github.com/ThePoultryMan/Particle-Moths)| • [dirtbar & co.](https://codeberg.org/ThePoultryMan/dirtbar) - Status configured at compile time.|
